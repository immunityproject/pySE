...
parser.add_argument('--debug', dest='debug', action='store_true',
                    help='Show information helpful for script debugging')
...
# where to find the jobs directories with FoldX results
...
def scan_protein(proteinName):
    ...
    for first in range(minIndex, maxIndex - window_size + 2):
        results.append(RangeResults(
            proteinName, first, first + window_size - 1))
    return results
...
def output_csv(results, has_epitopes, has_ranges):
    ...
    try:
        if has_epitopes:
            epitope_file = open(args.csv_filename +
        ...
        if args.csv_header:
            if has_epitopes:
                csv_epitopes.writerow(('protein', 'epitope', 'avg_entropy', 'max_entropy', 'max_2_entropy', 'max_3_entropy',
                                       'avg_displacement', 'min_displacement', 'min_2_displacement', 'min_3_displacement'))
            elif has_ranges:
                csv_epitopes.writerow(('protein', 'first', 'last', 'avg_entropy', 'max_entropy', 'max_2_entropy',
                                       'max_3_entropy', 'avg_displacement', 'min_displacement', 'min_2_displacement', 'min_3_displacement'))
...
def output_epitope_result_csv(er, out):
    ...
    entropies = sorted(
        [sr.entropy for sr in er.siteResults if sr.entropy is not None], reverse=True)
    while len(entropies) < 3:
        entropies += [None]
    avgD = er.averageDisplacement or ''
    displacements = sorted(
        [sr.averageDisplacement for sr in er.siteResults if sr.averageDisplacement is not None])
...
def output_range_result_csv(rr, out):
    ...
    entropies = sorted(
        [sr.entropy for sr in rr.siteResults if sr.entropy is not None], reverse=True)
    while len(entropies) < 3:
        entropies += [None]
    avgD = rr.averageDisplacement or ''
    displacements = sorted(
        [sr.averageDisplacement for sr in rr.siteResults if sr.averageDisplacement is not None])
    while len(displacements) < 3:
        displacements += [None]
        ...
        for site in result.siteResults:
            for mutation in site.evaluators:
                ...
                items = "{0},{1},{2},{3},{4}".format(
                    mutation.protein, mutation.site, mutation.wt, mutation.mutation, mutation.energyDelta)
...
class MutationEvaluator(object):
    ...
    def initialize(self):
        ...
        self.energyDeltas = [mean([e - w for e, w in zip(es, ws)])
                             for es, ws in zip(zip(*(self.energies)), zip(*(self.wtEnergies)))]
        ...
        if args.displacement:
            ...
            if len(self.displacements) == 0:
                print >> sys.stderr, 'Warning: no displacement data found for %s %s %s' % (
                    self.protein, self.site, self.mutation)
...
def initJobsDirs():
    ...
    pdbToProtein = {pdbPath.split(
        '/')[-1]: protein for protein, pdbPath in pdbs.iteritems()}
...
def checkJobsDir(pdbToProtein, dirname, filenames):
    ...
    with open(list_file_path) as list_file:
        ...
        if protein is None:
            raise Exception(
                '%s was not found in the pdbs variable in proteins.py' % pdbFilename)
    ...
    if mutation != wt or args.include_wt:
        ...
        if evaluatorKey in evaluators:
            print >> sys.stderr, 'Warning: duplicate data found for %s %d %s in directories %s and %s' % (
                protein, site, mutation, dirname, evaluators[evaluatorKey].directory)
        else:
            evaluators[evaluatorKey] = MutationEvaluator(
                dirname, protein, site, mutation, wt)
...
class SiteResults:
    ...
    def __init__(self, proteinName, site):
        ...
        # Displacements
        self.displacements = [
            ev.displacement for ev in self.evaluators if ev.displacement is not None]
        if len(self.displacements):
            self.averageDisplacement = mean(self.displacements)
        else:
            if args.displacement:
                print >> sys.stderr, 'Warning: no displacement data for %s %d' % (
...
class RangeResults:
    ...
    def __init__(self, proteinName, first, last):
        ...
        for site in range(first, last + 1):  # site ranges are inclusive by convention
            try:
                self.siteResults.append(
                    SiteResults.instance(proteinName, site))
            except Exception, e:
                print >> sys.stderr, 'Warning: ignoring %s %d due to errors' % (
                    proteinName, site)
        # Collect entropies from every site in the range
        entropies = [
            ev.entropy for ev in self.siteResults if ev.entropy is not None]
        if len(entropies) > 0:
        ...
        self.displacements = [
            ev.averageDisplacement for ev in self.siteResults if ev.averageDisplacement is not None]
        if len(self.displacements) > 0:
            self.averageDisplacement = mean(self.displacements)
        else:
            self.averageDisplacement = None
...
class EpitopeResults(RangeResults):
    ...
    def __init__(self, proteinName, epitopeName):
        ...
        if len(self.siteResults) != last - first + 1:
            print >> sys.stderr, 'Warning: only %d sites were found for %s %s' % (
...
class ProteinResults:
    ...
    def __init__(self, proteinName):
        ...
        self.epitopeResults = [EpitopeResults(
            proteinName, epitopeName) for epitopeName in protein]
...
def r_squared(pairs):  # pass a list of tuples [ (x1,y1), (x2,y2), … ]
    ...
    denominator = math.sqrt(sum(
        [x * x for x, y in residuals])) * math.sqrt(sum([y * y for x, y in residuals]))
    if denominator == 0:
        return float('NaN')
