...
parser.add_argument('--job',dest='job',default=None,help='only this one job directory will be processed, regardless of lockfile')
parser.add_argument('--jobs-dir',dest='jobs_dir',default='/epitopedata/jobs',help='parent directory of jobs directories')
parser.add_argument('--working-dir',dest='working_dir',default='foldx_job',help='where to copy job files before running FoldX')
parser.add_argument('--foldx-exe',dest='foldx_exe',default='FoldX.linux32',help='the name of the FoldX executable')
parser.add_argument('--foldx-dir',dest='foldx_dir',default='/epitopedata/ImmunityProject/foldx/foldx3b4hack_1',help='where the FoldX distribution files can be found')
parser.add_argument('--exit',dest='exit_when_done',action='store_true',help='if set, then the process exits after all jobs are completed')
args = None
...
def runFoldx(dirName):
	...
	proc = subprocess.Popen([args.foldx_exe,'-runfile','runfile.txt'],cwd=args.working_dir,executable=exe)
	result = proc.wait()
	...
	if result != 0:
		logging.error( 'FoldX returned code '+proc.returncode+' in '+dirName )
			...
			try:
				...
				try:
					with open(completed_filename, 'w') as lock_file:
						lock_file.write(gethostname() + '\n')
					logging.debug('Created completed file ' + completed_filename)
				except Exception as e:
					logging.warning('Could not write completed file ' + completed_filename)
			except Exception as e:
...
def setupWorkingDir():
	...
	chmod( path.join(args.working_dir,args.foldx_exe), S_IXUSR | S_IWUSR | S_IRUSR | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH )
